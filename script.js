...
function init() {
    ...
    // let countdown = Math.floor((targetDate - now) / 1000);

    let countdown = 1;

    const countdownElement = document.querySelector('.loading-init');
    const textWish = document.querySelector('.text-wish');
    ...
    const timer = setInterval(() => {
        ...
        if (countdown <= 0) {
            ...
            const fadeIn = setInterval(() => {
                opacity += 0.05;
                textWish.style.opacity = opacity.toFixed(2);
...
class Shell {
    ...
    burst(x, y) {
        ...
        if (typeof this.color === 'string') {
            ...
            throw new Error('Invalid shell color. Expected string or array of strings, but got: ' + this
                .color);
        }
